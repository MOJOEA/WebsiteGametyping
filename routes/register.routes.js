    ...
    body("useremail")
      ...
      .custom(async (value) => {
        const [rows] = await dbConnection.query(
          "SELECT email_account FROM account WHERE email_account = ?",
          [value]
        );
  ...
  async (req, res) => {
    ...
    try {
      ...
      await dbConnection.query(
        `INSERT INTO account
        (userID, username_account, email_account, password_account, salt_account, role_account, images_account, login_count_account, lock_account, ban_account)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [null, req.body.username, req.body.useremail, hashedPassword, '', 'member', 'default_images_account.jpg', 0, 0, 0]
      );

      return res.send("Register สำเร็จ!");
    } catch (err) {
      console.error(err);
